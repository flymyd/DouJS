 ...
 */
export const dealCards = (arr, playerNum = 3, holeCardsNum = 3) => {
  const result = {
    cards: [],
    holeCards: []
  }
 ...
 * @param cardArr 牌组
 */
export const parseArrToCards = (cardArr) => {
 ...
 * @param arr
 */
export function classifyAndCount(arr) {
  let countMap = {};
  let result = {};
  arr.forEach((item) => {
 ...
 * @param playerNum
 */
export function initHand(playerNum) {
  // 拿牌 洗牌 切分 分别排序 赋值
  let originalCardHeap = []
  if (playerNum === 3) {
